import { PurchaseOrderDtoForInsertion, PurchaseOrderDtoForUpdate, PurchaseOrderLineDtoForInsertion, PurchaseOrderLineDtoForUpdate } from "@/api/apiDtos";
...
export const PurchaseOrderForm = <T extends PurchaseOrderDtoForInsertion | PurchaseOrderDtoForUpdate>({
    ...
    type AnyLine = PurchaseOrderLineDtoForInsertion | PurchaseOrderLineDtoForUpdate;
    ...
    const setLines = (updater: (prev: AnyLine[]) => AnyLine[]) => {
        ...
        ({
            ...prev,
            purchaseOrderLine: updater(((prev as any).purchaseOrderLine ?? []) as AnyLine[]),
        } as T)
        );
    };
            ...
            <div className="mt-6">
                ...
                {lines.length === 0 ? (
                    ...
                    <div className="space-y-3">
                        {lines.map((line, idx) => (
                            <div key={(line as any).id ?? idx} className="border rounded p-3">
                                <div className="grid grid-cols-3 gap-3">
                                    ...
                                    <div>
                                        ...
                                        <input
                                            value={line.malzemeAdi ?? ""}
                                            onChange={(e) => updateLine(idx, "malzemeAdi", e.target.value as any)}
                                            className="w-full border p-2 rounded"
                                        />
                                    </div>
