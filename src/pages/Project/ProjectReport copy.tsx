...
import { FaPencilAlt, FaShower, FaTrash, FaUpload } from "react-icons/fa";
      ...
      content(close) {
        let fields: FieldDefinition[] = [
          ...
          },
          {
            ...
            name: "hazirlayan",
            label: "Hazırlayan",
            options: personels.map<FieldOption>((x) => ({
          ...
          {
            defaultValue: null,
            clickIcon: [<FaUpload title="Yeni Dosya Ekle" />,<FaShower title="Önizle" />],
            type: "button",
            name: "btn",
        ...
        if (
          (record?.dosyalar &&
            record?.dosyalar.length > 0 &&
            fields.filter((x) => x.group == "Dosyalar").length == 0) ||
          true
        ) {
